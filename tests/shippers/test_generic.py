...
async def test_ups_delivered_class(hass, mock_imap_ups_delivered):
    ...
    with (
        ...
        result = await shipper.process(mock_imap_ups_delivered, "today", "ups_delivered")
...
async def test_fedex_delivered_class(hass, mock_imap_fedex_delivered_with_photo):
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.generic.Path.mkdir"),
        patch("custom_components.mail_and_packages.shippers.generic.generic_delivery_image_extraction", return_value=True),
    ):
        result = await shipper.process(mock_imap_fedex_delivered_with_photo, "today", "fedex_delivered")
        assert result[ATTR_COUNT] == 1
...
async def test_usps_delivered_class(hass, mock_imap_usps_delivered_individual):
    ...
    with (
        ...
        result = await shipper.process(mock_imap_usps_delivered_individual, "today", "usps_delivered")
        assert result[ATTR_COUNT] == 1
...
async def test_usps_exception_class(hass, mock_imap_usps_exception):
    ...
    with (
        ...
        result = await shipper.process(mock_imap_usps_exception, "today", "usps_exception")
        assert result[ATTR_COUNT] == 1
...
async def test_generic_with_images_and_amazon_mentions(hass):
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.generic.Path.mkdir"),
        patch("custom_components.mail_and_packages.shippers.generic.generic_delivery_image_extraction", return_value=True),
        patch("custom_components.mail_and_packages.shippers.generic.get_tracking", new_callable=AsyncMock, return_value=["1Z123"]),
        patch("custom_components.mail_and_packages.shippers.generic.find_text", new_callable=AsyncMock, return_value=1),
    ):
...
async def test_generic_ups_exception(hass):
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.generic.get_tracking", new_callable=AsyncMock, return_value=["1Z999"]),
    ):
...
async def test_generic_multiple_emails(hass):
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.generic.get_tracking", new_callable=AsyncMock, return_value=["1Z123", "1Z456"]),
    ):
        result = await shipper.process(mock_account, "today", "ups_delivered")
