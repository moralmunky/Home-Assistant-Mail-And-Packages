...
async def test_informed_delivery_emails_class(
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.usps.anyio.Path.is_dir", return_value=True),
        ...
        patch("custom_components.mail_and_packages.shippers.usps.io_save_file", new_callable=AsyncMock),
        patch("custom_components.mail_and_packages.shippers.usps.resize_images", return_value=["test/path/usps/img1.jpg"]),
        patch("custom_components.mail_and_packages.shippers.usps.generate_delivery_gif"),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
    ):
        result = await shipper.process(mock_imap_usps_informed_digest, "today", "usps_mail")
...
async def test_new_informed_delivery_emails_class(
    ...
    with (
        ...
        result = await shipper.process(mock_imap_usps_new_informed_digest, "today", "usps_mail")
...
async def test_informed_digest_no_mail_class(
    ...
    with (
        ...
        patch("custom_components.mail_and_packages.shippers.usps.cleanup_images"),
        patch("custom_components.mail_and_packages.shippers.usps.copy_overlays"),
        patch("custom_components.mail_and_packages.shippers.usps.copyfile"),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
    ):
        result = await shipper.process(mock_imap_usps_informed_digest_no_mail, "today", "usps_mail")
...
async def test_informed_delivery_with_images_class(hass):
    ...
    with (
        ...
        patch("custom_components.mail_and_packages.shippers.usps.generate_delivery_gif"),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
        patch("custom_components.mail_and_packages.shippers.usps.random_filename", return_value="random.jpg"),
    ):
...
async def test_informed_delivery_placeholder_image(hass):
    ...
    mock_account.fetch.return_value = MagicMock(result="OK", lines=[b"RFC822", msg_bytes])

    with (
        patch("custom_components.mail_and_packages.shippers.usps.anyio.Path.is_dir", return_value=True),
        patch("custom_components.mail_and_packages.shippers.usps.cleanup_images"),
        patch("custom_components.mail_and_packages.shippers.usps.copy_overlays"),
        patch("custom_components.mail_and_packages.shippers.usps.Path.exists", return_value=True),
        patch("custom_components.mail_and_packages.shippers.usps.resize_images", return_value=["test/path/usps/img1.jpg"]),
        patch("custom_components.mail_and_packages.shippers.usps.generate_delivery_gif"),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
    ):
...
async def test_informed_delivery_announcement_filtering(hass):
    ...
    with (
        ...
        patch("custom_components.mail_and_packages.shippers.usps.copy_overlays"),
        patch("custom_components.mail_and_packages.shippers.usps.io_save_file", new_callable=AsyncMock),
        patch("custom_components.mail_and_packages.shippers.usps.random_filename", side_effect=filenames),
        patch("custom_components.mail_and_packages.shippers.usps.resize_images", side_effect=lambda imgs, w, h: imgs),
        patch("custom_components.mail_and_packages.shippers.usps.generate_delivery_gif"),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
    ):
        result = await shipper.process(mock_account, "today", "usps_mail")
...
async def test_informed_delivery_mkdir_error(hass):
    ...
    with (
        patch("custom_components.mail_and_packages.shippers.usps.anyio.Path.is_dir", return_value=False),
        patch("custom_components.mail_and_packages.shippers.usps.anyio.Path.mkdir", side_effect=OSError("Permission denied")),
    ):
        result = await shipper.process(mock_account, "today", "usps_mail")
        assert result[ATTR_COUNT] == 0
...
async def test_informed_delivery_resize_error(hass):
    ...
    with (
        ...
        patch("custom_components.mail_and_packages.shippers.usps.Path.exists", return_value=True),
        patch("custom_components.mail_and_packages.shippers.usps.resize_images", return_value=["test/path/usps/img1.jpg"]),
        patch("custom_components.mail_and_packages.shippers.usps.generate_delivery_gif", side_effect=ValueError("Invalid image")),
        patch("custom_components.mail_and_packages.shippers.usps.get_formatted_date", return_value="25-Sep-2020"),
    ):
        result = await shipper.process(mock_account, "today", "usps_mail")
