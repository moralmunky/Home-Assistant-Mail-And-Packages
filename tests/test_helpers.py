...
async def test_update_time():
    assert update_time() == datetime.datetime.now(timezone.utc).isoformat(timespec='minutes')
...
async def test_process_emails(
    ...
    assert result["mail_updated"] == datetime.datetime(2022, 1, 6, 12, 14, tzinfo=datetime.timezone.utc)
