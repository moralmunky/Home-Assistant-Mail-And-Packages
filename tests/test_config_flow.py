...
async def test_form_invalid_custom_img_path(hass: HomeAssistant):
    """Test we handle invalid custom image path."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    assert result["type"] == "form"
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            ...
            "imap_timeout": 30,
            "scan_interval": 20,
            "resources": ["mail_updated", "usps_delivered"],
        },
    )
    ...
    # Test invalid custom image path
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "custom_img_file": "/invalid/path/image.gif",
        },
    )

    assert result["type"] == "form"
    assert result["errors"] == {"custom_img_file": "invalid_path"}
...
async def test_form_invalid_ffmpeg(hass: HomeAssistant):
    """Test we handle invalid ffmpeg path."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            ...
            "folder": '"INBOX"',
            "generate_grid": False,
            "generate_mp4": True,  # Enable MP4 generation
            "gif_duration": 5,
            "imap_timeout": 30,
            "scan_interval": 20,
            "resources": ["mail_updated", "usps_delivered"],
    ...
    # Mock ffmpeg check to return False (invalid)
    with patch(
        "custom_components.mail_and_packages.helpers._check_ffmpeg",
        return_value=False,
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
    ...
    assert result["type"] == "form"
    assert result["errors"] == {"generate_mp4": "invalid_ffmpeg"}


async def test_form_index_error(hass: HomeAssistant):
    """Test we handle index error during validation."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    with patch(
        "custom_components.mail_and_packages.helpers._test_login",
        side_effect=IndexError("Test index error"),
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
    ...
    assert result["type"] == "form"
    assert result["errors"] == {"base": "index_error"}


async def test_form_index_error_2(hass: HomeAssistant):
    """Test we handle index error during validation step 2."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    # Mock an index error during step 2 validation
    with patch(
        "custom_components.mail_and_packages.config_flow._validate_user_input",
        side_effect=IndexError("Test index error in step 2"),
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
...
async def test_form_mailbox_format2(hass: HomeAssistant):
    """Test we handle mailbox format error."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    # Test with invalid folder format
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "allow_external": False,
            "custom_img": False,
            "folder": "INBOX",  # Missing quotes
            "generate_grid": False,
            "generate_mp4": False,
            "gif_duration": 5,
            "imap_timeout": 30,
    ...
    assert result["type"] == "form"
    assert result["errors"] == {"folder": "mailbox_format"}


async def test_form_mailbox_format3(hass: HomeAssistant):
    """Test we handle another mailbox format error."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    # Test with invalid folder format (empty)
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "allow_external": False,
...
async def test_imap_login_error(hass: HomeAssistant):
    """Test we handle IMAP login errors."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
async def test_form_amazon_error(hass: HomeAssistant):
    """Test we handle Amazon configuration errors."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            ...
            "imap_timeout": 30,
            "scan_interval": 20,
            "resources": ["amazon_packages", "usps_delivered"],
        },
    )

    # Test with invalid Amazon domain
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "amazon_domain": "",  # Empty domain
            "amazon_days": 3,
            "amazon_fwds": "fakeuser@test.email",
        },
    )
...
async def test_form_amazon_error_2(hass: HomeAssistant):
    """Test we handle another Amazon configuration error."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    # Test with invalid Amazon forwards
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "amazon_domain": "amazon.com",
            "amazon_days": 3,
            "amazon_fwds": "invalid_email",  # Invalid email format
        },
    )

    assert result["type"] == "form"
    assert result["errors"] == {"amazon_fwds": "invalid_email"}



async def test_form_storage_error(hass: HomeAssistant):
    """Test we handle storage path errors."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
async def test_reconfigure(
    ...
    # Sort any lists in the data for comparison
...
async def test_reconfigure_no_amazon_no_custom_image(
    ...
    """Test reconfigure flow with no Amazon sensors and no custom images."""
    entry = hass.config_entries.async_entries(DOMAIN)[0]
    ...
    # Should skip Amazon config since no Amazon sensors are selected
    assert result["type"] == "form"
    assert result["step_id"] == step_id_3
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], input_3
...
async def test_reconfig_no_cust_img(
    ...
    with patch(
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], {"storage": ".storage/mail_and_packages/images"}
        )
...
@pytest.mark.parametrize(
    "input_1,step_id_2,input_2,step_id_3,input_4,step_id_5,input_5,title,data",
    [
        (
            {
                "host": "imap.test.email",
