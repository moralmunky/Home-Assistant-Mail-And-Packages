...
async def test_find_text_async():
    ...
    with patch("custom_components.mail_and_packages.utils.email.email_fetch", new_callable=AsyncMock) as mock_fetch:
...
async def test_email_fetch_success():
    ...
    mock_res = MagicMock()
    mock_res.result = "OK"
    mock_res.lines = [(b"1 (RFC822 {100}", b"From: test@example.com\nSubject: Test\n\nBody content")]
...
async def test_login_success():
    ...
    mock_hass = MagicMock()
    with patch("custom_components.mail_and_packages.utils.imap.IMAP4_SSL") as mock_imap_ssl:
        mock_acc = AsyncMock()
...
async def test_login_oauth_success():
    ...
    with patch("custom_components.mail_and_packages.utils.imap.IMAP4_SSL") as mock_imap_ssl:
        ...
        result = await login(mock_hass, "host", 993, "user", None, "SSL", oauth_token="token")
        assert result == mock_acc
...
async def test_login_no_verify():
    ...
    mock_hass = MagicMock()
    with (
        patch("custom_components.mail_and_packages.utils.imap.IMAP4_SSL") as mock_imap_ssl,
