...
from custom_components.mail_and_packages.utils.amazon_image import (
    _MAX_IMAGE_SIZE,
...
def amazon_date_search(email_msg: str, patterns: list[str] | None = None) -> int:
    ...
    for pattern in patterns or AMAZON_TIME_PATTERN_END:
...
def amazon_date_regex(email_msg: str, patterns: list[str] | None = None) -> str | None:
    ...
    for pattern in patterns or AMAZON_TIME_PATTERN_REGEX:
        if (found := re.search(pattern, email_msg, re.IGNORECASE)) and found.groups():
