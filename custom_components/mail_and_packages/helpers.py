...
def process_emails(hass: HomeAssistant, config: ConfigEntry) -> dict:
    ...
    # Calculate total package sensors
...
def _calculate_total_sensors(data: dict, resources: list) -> None:
    ...
    Args:
        ...
        resources: List of enabled sensor resources
    """
    # Only calculate if total sensors are enabled
    if not any(sensor in ["total_packages", "total_delivered", "total_delivering"] for sensor in resources):
        return
    
    # Define carrier sensor patterns
    package_sensors = [
...
def decode_subject_line(subject: str) -> str:
    ...
    Args:
        subject: Raw subject line from email
    ...
    try:
        # Handle UTF-8 encoded subjects like "=?UTF-8?Q?=F0=9F=8C=8D_Ihre_AliExpress_Sendung_liegt_im_Briefkasten?="
    ...
    for i in mail_list:
        ...
        for response_part in data:
            if isinstance(response_part, tuple):
                ...
                if email_subject:
                    email_subject = decode_subject_line(str(email_subject))
